...
interface Props {
  ...
  defaultTemplate: 'minimal' | 'bold' | 'creative'
}

const TEMPLATES = ['minimal', 'bold', 'creative'] as const
const LABELS: Record<string, string> = { minimal: '☀', bold: '🌙', neutral: '📄' }

export default function DemoToggle({ content, defaultTemplate }: Props) {
  const [template, setTemplate] = useState<'minimal' | 'bold' | 'creative'>(defaultTemplate)
  ...
  return (
    <>
      ...
      {template === 'creative' && <Creative content={content} />}
      ...
      <button
        ...
        style={{
          background: template === 'bold' ? '#F0F6FF' : template === 'creative' ? '#0d0d0d' : '#0A0A0A',
