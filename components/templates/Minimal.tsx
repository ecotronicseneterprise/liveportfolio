          ...
          <div className="sm:hidden border-t border-gray-100 px-6 py-3 space-y-1 bg-white">
            {navLinks.map((link) => (
              <a
                ...
                className="block py-2 text-sm text-gray-600 hover:text-[#1D9E75] transition-colors"
              >
                {link}
              </a>
            ))}
      ...
      <main className="max-w-[680px] mx-auto px-6">
        ...
        <section className="pt-20 pb-16 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-xs text-[#1D9E75] font-medium">Available for opportunities</span>
          </div>
          ...
          <div className="flex items-start gap-6 mb-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1
                className="text-4xl sm:text-5xl font-bold tracking-tight mt-1 leading-tight"
                style={{ fontFamily: 'var(--font-playfair), Georgia, serif' }}
              >
                {content.name}
              </h1>
              {/* Teal divider */}
              <div className="w-8 h-0.5 bg-[#1D9E75] my-3" />
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6">
            <a href={`mailto:${content.email}`} className="text-sm text-[#1D9E75] hover:underline font-medium">
              {content.email}
            </a>
            {content.github_url && (
              <a href={content.github_url} target="_blank" rel="noopener noreferrer" className="text-sm text-[#1D9E75] hover:underline font-medium">
                GitHub →
              </a>
            )}
            {content.linkedin_url && (
              <a href={content.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-sm text-[#1D9E75] hover:underline font-medium">
                LinkedIn →
              </a>
            )}
          </div>
                    ...
                    <div key={i}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {group.items.map((skill) => (
                          <span
                            key={skill}
                            className="px-3 py-1 text-sm bg-gray-50 border border-gray-200 rounded-full text-gray-700 hover:border-[#1D9E75] hover:text-[#1D9E75] hover:scale-105 transition-all"
                          >
                            {skill}
                          </span>
                        ))}
              ...
              <div className="space-y-8">
                {content.experience.map((exp, i) => (
                  <div key={i} className="relative pl-4 border-l-2 border-gray-100">
                    <div className="absolute -left-[5px] top-1 w-2 h-2 rounded-full bg-[#1D9E75]" />
                    <div className="flex flex-wrap items-baseline gap-2 mb-1">
                      <h3 className="font-semibold text-[#0A0A0A]">{exp.role}</h3>
                      <span className="text-gray-400 text-sm">at {exp.company}</span>
                    </div>
                    ...
                    <ul className="space-y-1.5">
                      {exp.bullets.map((bullet, j) => (
                        <li key={j} className="text-sm text-gray-600 leading-relaxed flex gap-2">
                          <span className="text-[#1D9E75] mt-1 flex-shrink-0">•</span>
                          <span>{bullet}</span>
                        </li>
                      ))}
                    </ul>
          ...
          <div className="sm:grid sm:grid-cols-[120px_1fr] sm:gap-12">
            ...
            <div>
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#0A0A0A] text-white text-sm font-medium rounded-full hover:bg-[#1D9E75] hover:gap-3 transition-all duration-200"
              >
                Get in touch →
              </a>
            </div>
        ...
        <footer className="py-8 text-center">
          <a
            ...
            className="text-xs text-gray-300 hover:text-[#1D9E75] transition-colors"
            target="_blank"
            rel="noopener noreferrer"
          >
